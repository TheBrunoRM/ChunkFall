...
import me.brunorm.chunk.ChunkFall;

public class BukkitReflection {

	String prefix = "&7[&cNMS-Debug&7]";

	String version = ChunkFall.get().getServerPackageVersion();
	...
	public Class<?> getEnumTitleAction() {
		try {
			this.debug(this.prefix + " &6Loading &cold &bEnumTitleAction");
			return this.getNMS("EnumTitleAction");
		} catch (final Exception e) {
			try {
				this.debug(this.prefix + " &6Loading &anew &bEnumTitleAction");
				return this.getNMS("PacketPlayOutTitle$EnumTitleAction");
			} catch (final Exception e2) {
				this.debug(this.prefix + " &cCould not load &bEnumTitleAction");
				return null;
			}
		}
	}
	...
	public Class<?> getChatMessageTypeClass() {
		try {
			this.debug(this.prefix + " &6Loading &bChatMessageType");
			return this.getNMS("ChatMessageType");
		} catch (final Exception e) {
			this.debug(this.prefix + " &cCould not find &bChatMessageType");
			return null;
		}
	}
	...
	public void sendParticles(Location loc, String particle, int amount) {
		...
		try {
			...
			try {
				...
				this.debug("Could not spawn particles.");
			}
		}
	}
